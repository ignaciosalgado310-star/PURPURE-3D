...
public final class ClientPurpureEffects {
    ...
    private static final ResourceLocation BLUE_TEX=new ResourceLocation(PurpureMod.MODID,"textures/effect/blue_energy.png");
    private static final ResourceLocation RED_TEX=new ResourceLocation(PurpureMod.MODID,"textures/effect/red_energy.png");
    private static final ResourceLocation PURPLE_TEX=new ResourceLocation(PurpureMod.MODID,"textures/effect/purple_energy.png");
    ...
    private static void drawFX(PoseStack p,FX f,float t){
        if(t<235){
            ...
            orb(p,Mth.cos(a)*r,y,Mth.sin(a)*r,size,BLUE_TEX,0.05f,.30f,1f,t,-1);
            orb(p,Mth.cos(a+Mth.PI)*r,y,Mth.sin(a+Mth.PI)*r,size,RED_TEX,1f,.045f,.03f,t,1);
        }
        ...
        if(t>=FUSION_START){
            ...
            sphere(p,base*.28f,PURPLE_TEX,.98f,.96f,1f,.70f);
            sphere(p,base*.50f,PURPLE_TEX,.95f,.58f,1f,.48f);
            sphere(p,base*.72f,PURPLE_TEX,.70f,.12f,1f,.32f);
            sphere(p,base,PURPLE_TEX,.28f,.02f,.82f,.20f);
    ...
    private static void orb(PoseStack p,float x,float y,float z,float R,ResourceLocation tex,float r,float g,float b,float t,int dir){
        p.pushPose();p.translate(x,y,z);
        float pulse=R*(1+.07f*Mth.sin(t*.24f));
        sphere(p,pulse*.34f,tex,.98f,.99f,1f,.62f);
        sphere(p,pulse*.62f,tex,r,g,b,.46f);
        sphere(p,pulse*.84f,tex,r*.82f,g*.82f,b,.30f);
        sphere(p,pulse,tex,r*.58f,g*.58f,b,.18f);
    ...
    private static void sphere(PoseStack p,float R,ResourceLocation tex,float r,float g,float b,float a){
        RenderSystem.setShader(GameRenderer::getPositionTexColorShader);
        RenderSystem.setShaderTexture(0,tex);
        Matrix4f m=p.last().pose();BufferBuilder v=Tesselator.getInstance().getBuilder();v.begin(VertexFormat.Mode.QUADS,DefaultVertexFormat.POSITION_TEX_COLOR);
        int lon=40,lat=22;
        for(int iy=0;iy<lat;iy++){
            float p0=((float)iy/lat-.5f)*Mth.PI,p1=((float)(iy+1)/lat-.5f)*Mth.PI;
            float vv0=(float)iy/lat,vv1=(float)(iy+1)/lat;
            for(int ix=0;ix<lon;ix++){
                float t0=ix*PI2/lon,t1=(ix+1)*PI2/lon;
                float u0=(float)ix/lon,u1=(float)(ix+1)/lon;
                STV(v,m,R,p0,t0,u0,vv0,r,g,b,a);
                STV(v,m,R,p0,t1,u1,vv0,r,g,b,a);
                STV(v,m,R,p1,t1,u1,vv1,r,g,b,a);
                STV(v,m,R,p1,t0,u0,vv1,r,g,b,a);
            }
        }
        BufferUploader.drawWithShader(v.end());
    }

    private static void STV(BufferBuilder v,Matrix4f m,float R,float p,float t,float u,float vv,float r,float g,float b,float a){
        v.vertex(m,R*Mth.cos(p)*Mth.cos(t),R*Mth.sin(p),R*Mth.cos(p)*Mth.sin(t)).uv(u,vv).color(C(r),C(g),C(b),C(a)).endVertex();
    }
